        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function getVisibleRows() {
                const rows = Array.from(document.querySelectorAll('#recordsPatientsTable tbody tr:not(.no-results):not(.no-patients)'));
            ...
            function updateTable() {
                const rows = Array.from(document.querySelectorAll('#recordsPatientsTable tbody tr:not(.no-results):not(.no-patients)'));
                const noPatientsRow = document.querySelector('#recordsPatientsTable tbody tr.no-patients');
                ...
                document.getElementById('clearFiltersBtnRecords').style.display = isFiltered ? 'block' : 'none';
                ...
                // Update no results/no patients message
                const noResults = document.getElementById('noResultsRecords');
                if (noPatientsRow) {
                    noPatientsRow.style.display = rows.length === 0 && !isFiltered ? '' : 'none';
                }
            ...
            window.addPatientRecord = function() {
                window.location.href = "{{ route('prenatals') }}";
            };
